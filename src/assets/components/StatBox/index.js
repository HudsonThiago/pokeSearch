...
export default function AbilityBox({stat}){
    ...
    const getColor=()=>{
        switch(stat.stat.name){
            ...
            case 'special-attack':
                setColor("#66FFA3");
                setName("Special Attack");
                break;
            case 'special-defense':
                setColor("#66B6FF");
                setName("Special Defense");
                break;
            case 'speed':
                setColor("#FF7BAB");
                setName("Speed");
    ...
    return (
        <div className="statBox">
            ...
            <div className="statBoxMain">
                <h3>{name} - {stat.base_stat}</h3>
                <div className="statBoxOutBar"><div className="statBoxBar" style={{backgroundColor: color, width: percentage+'%' }}></div></div>
            </div>
        </div>
    )
}